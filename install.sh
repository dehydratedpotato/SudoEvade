...
while getopts "uirhV" OPT; do
    case $OPT in
        u) echo "${BOLD}Uninstalling SudoEvade ($OS $VER)${NF}"; uninstall 2> /dev/null; exit ;;
        i) echo "${BOLD}Installing SudoEvade ($OS $VER) ${NF}"; install 2> /dev/null; exit ;;
        r) echo "${BOLD}Reinstalling SudoEvade ($OS $VER)${NF}"; uninstall 2>  /dev/null; install 2>  /dev/null; exit;;
